...
namespace Microsoft.EntityFrameworkCore.Couchbase.Query.Expressions.Internal
{
    public class SelectExpression : Expression
    {
        ...
        public BinaryExpression GetDiscriminatorPredicate(IEntityType entityType)
        {
            ...
            var discriminatorProperty = entityType.Couchbase().DiscriminatorProperty;

            var discriminatorPredicate = Equal(
                new KeyAccessExpression(discriminatorProperty, FromExpression),
                Constant(concreteEntityTypes[0].Couchbase().DiscriminatorValue, discriminatorProperty.ClrType));
            ...
            if (concreteEntityTypes.Count > 1)
            {
                discriminatorPredicate
                    = concreteEntityTypes
                        ...
                        .Select(
                            concreteEntityType
                                => Constant(concreteEntityType.Couchbase().DiscriminatorValue, discriminatorProperty.ClrType))
                        .Aggregate(
