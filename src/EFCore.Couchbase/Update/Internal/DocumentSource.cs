...
namespace Microsoft.EntityFrameworkCore.Couchbase.Update.Internal
{
    public class DocumentSource
    {
        private readonly string _collectionId;
        private readonly CouchbaseDatabaseWrapper _database;
        private readonly IProperty _idProperty;

        public DocumentSource(IEntityType entityType, CouchbaseDatabaseWrapper database)
        {
            _collectionId = entityType.Couchbase().ContainerName;
            _database = database;
        ...
        public JObject CreateDocument(IUpdateEntry entry)
        {
            ...
            foreach (var ownedNavigation in entry.EntityType.GetNavigations())
            {
                ...
                var nestedPropertyName = fk.DeclaringEntityType.Couchbase().ContainingPropertyName;
                if (nestedValue == null)
                {
        ...
        private static JToken ConvertPropertyValue(IProperty property, object value)
        {
            ...
            return (value as JToken) ?? JToken.FromObject(value, CouchbaseClientWrapper.Serializer);
        }
    }
}
